...
var MatchBetsMaster = function (matchBet) {
...
MatchBetsMaster.addMatchBet = async function (
  matchBet,
  main_type,
  market_name,
  enable_draw
  ...
  try {
    matchBet.event_name = market_name;
    ...
    const qry_insert = "INSERT INTO matchbets set ?";
...
MatchBetsMaster.getMatchBetByEventId = async function (event_id, user_id) {
  try {
    let sessiobets = await new Promise((resolve, reject) => {
      dbConn.query(
        "select * from matchbets where event_id = ? and user_id=? order by updated_at desc",
        [event_id, user_id],
        (err, res) => {
          if (err) {
            reject(err);
...
MatchBetsMaster.getMatchBetsByUserID = async function (user_id) {
  try {
...
MatchBetsMaster.getMatchBetHistoryByEventId = async function (event_id) {
  try {
...
MatchBetsMaster.getMatchBetByEventIdByUserID = async function (
  event_id,
  user_id
) {
  try {
    let matchbets = await new Promise((resolve, reject) => {
      dbConn.query(
        "select * from matchbets where user_id = ? and event_id = ? and status = 1 order by updated_at desc LIMIT 1",
        [user_id, event_id],
        (err, res) => {
          if (err) {
            reject(err);
...
MatchBetsMaster.getMatchBetByFilter = async function (user_id, from, to) {
  try {
    let matchbets = await new Promise((resolve, reject) => {
      dbConn.query(
...
MatchBetsMaster.getOpenMatchBetsInadmin = async function (user_id) {
  try {
    let matchbets = await new Promise((resolve, reject) => {
      dbConn.query(
        "select mb.* from matchbets mb where mb.user_id = ? AND (mb.updated_at >= DATE_SUB(NOW(), INTERVAL 5 DAY) OR mb.status=0 ) ;",
        [user_id],
        (err, res) => {
          if (err) {
            reject(err);
...
MatchBetsMaster.getOpenMatchBets = async function (user_id,status=0,days=3) {
  try {
    ...
    let matchbets = await new Promise((resolve, reject) => {
      dbConn.query(
        "select * from matchbets where user_id = ? AND status = ? AND updated_at >= DATE(NOW() - INTERVAL ? DAY) ORDER BY id DESC",
        [user_id,status,days],
        (err, res) => {
          if (err) {
            reject(err);
...
MatchBetsMaster.getBetPlacedMatches = async function () {
  try {
...
MatchBetsMaster.getAllBetsByEventId = async function (event_id) {
  try {
    let betplaced_matches = await new Promise((resolve, reject) => {
      dbConn.query(
...
MatchBetsMaster.getMatchBetsPlaced = async function (creator_id, event_id) {
  try {
    ...
    if (role_id == 0) {
      qry =
        "SELECT u.username, m.* FROM `matchbets` m join users as u on u.id = m.user_id WHERE m.event_id = ? order by m.updated_at desc ";
      values = [event_id];
    } else {
      qry =
        "SELECT u.username, m.* FROM `matchbets` m join users as u on u.id = m.user_id WHERE FIND_IN_SET(?,creator_id) and m.event_id = ? order by m.updated_at desc ";
...
MatchBetsMaster.getAdminExp = async function (admin_id, event_id) {
  try {
...
MatchBetsMaster.getLatestMatchBet = async function (creator_id, event_id) {
  try {
...
MatchBetsMaster.getSumExpByEvent = async function (creator_id, event_id) {
  try {
    const sum_exp_event = await new Promise((resolve, reject) => {
      ...
      dbConn.query(
        "SELECT SUM(e.exp_amount1) AS sum_exp_amount1, SUM(e.exp_amount2) AS sum_exp_amount2, SUM(e.exp_amount3) AS sum_exp_amount3 FROM exposures e JOIN users u ON e.user_id = u.id WHERE FIND_IN_SET(?, u.creator_id) > 0 and e.event_id = ? and e.main_type='match_odd'",
        [creator_id, event_id],
        (err, res) => {
          if (err) {
            reject(err);
...
MatchBetsMaster.getSumExpInPlayEvents = async function (creator_id) {
  try {
    let qry = "";
...
MatchBetsMaster.getUserExpStatus = async function (creator_id, event_id) {
  try {
...
MatchBetsMaster.deleteMatchbets = async function (ids) {
  try {
    let bets_added = await new Promise((resolve, reject) => {
      dbConn.query(
        "INSERT INTO deletedmatchbets(bet_id,user_id,event_id,market_id,event_name,main_type,runner_name,type,price,size,bet_amount,loss_amount,win_amount,exp_amount1,exp_amount2,exp_amount3,profit_loss,status,is_won,is_switched,created_at,updated_at) SELECT * FROM matchbets where id IN (?)",
        [ids],
        (err, res) => {
          if (err) {
            reject(err);
